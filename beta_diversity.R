...
metadata2 <- read.delim("metadata_assignment2.txt", sep = "\t", header = T, quote = "", stringsAsFactors = F)
metadata2[1,]
metadata2[,1]
metadata2[1:3,1:3]
...
metadata<-read_q2metadata("metadata_assignment2.txt")
str(metadata)
levels(metadata$`Affection`)
...
body_colors1 <- c("Blue", "Green", "Gray", "Black", "Red")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=Affection)) +
  geom_point() + #alpha controls transparency and helps when points are overlapping
  theme_q2r() +
  xlab("PC1 (32.27%)") +
  ylab("PC2 (22.28%)") +
  scale_color_manual(values=c("Blue", "Green"), name = "Affection")

# Now we are going to make our code a little more re-usable
my_column <- "Affection"
#my_column <- "DietTreatment"

#Generate a plot for Bray curtis
...
ggsave(paste0("output/BC-basic_", my_column,".tiff"), height=3, width=6, device="tiff") # save a PDF 3 inches by 4 inches

centroids <- aggregate(cbind(PC1,PC2)~get(my_column),bc_meta,mean)
colnames(centroids)[1] <- "Affection"
...
#The same Bray curtis plot but with figures

ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  geom_point(aes(shape= Herd)) + #alpha controls transparency and helps when points are overlapping
  #geom_point(data=centroids, size = 3) +
  theme_q2r() +
  #stat_ellipse(level = 0.95, type = "t") +
  xlab(paste0("PC1 (", round(100*bc_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=body_colors, name = "Affection")
ggsave(paste0("output/Wuni-ellipse_", my_column,".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#Same weighted unifrac but with figures
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  geom_point(aes(shape= Herd), size = 3) + #alpha controls transparency and helps when points are overlapping
  #geom_point(data=centroids, size = 3) +
  theme_q2r() +
  stat_ellipse(level = 0.95, type = "t") +
  xlab(paste0("PC1 (", round(100*Wuni_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Affection")
ggsave(paste0("output/Wuni-ellipse_", my_column,"-Herd.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches


#Same but with Unweighted unifrac
...
ggplot(jaccard_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Affection")
ggsave(paste0("output/jaccard-ellipse_", my_column,".pdf"), height=3, width=4.5, device="pdf")
...
PERMANOVA_out <- adonis2(bc_dm ~ Affection, data = metadata_sub)
