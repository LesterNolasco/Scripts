...
meta<-read_q2metadata("metadata_assignment2.txt")
str(meta)
...
chao1<-read_qza("core-metrics-results/chao1_vector.qza")
chao1<-chao1$data %>% rownames_to_column("SampleID") # this moves the sample names to a new column that matches the metadata and allows them to be merged
...
alpha_diversity = merge(x=alpha_diversity, y=evenness, by.x = "SampleID", by.y = "SampleID")
...
meta$affection.ord = factor(meta$Affection, c("low", "high"))
levels(meta$affection.ord)

#Plot
boxplot(observed_features ~ affection.ord, data=meta, ylab="Observed Features")

observed_features_boxplot <- ggplot(meta, aes(affection.ord, observed_features)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
...
kruskal.test(faith_pd ~ Affection, data=meta)
kruskal.test(chao1 ~ Affection, data=meta)
kruskal.test(observed_features ~ Affection, data=meta)
kruskal.test(shannon_entropy ~ Affection, data=meta)
kruskal.test(pielou_evenness ~ Affection, data=meta)
...
#Plot
boxplot(faith_pd ~ Affection, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

#Create the alpha diversity plot for faith

faith_pd_bp <- ggplot(meta, aes(Affection, faith_pd)) + 
  geom_boxplot(aes(color = Affection)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Faith Phylogenetic Diversity", x = "") 
ggsave("output/pd.png", faith_pd_bp, height = 3, width = 3)

# For chao1
chao1_bp <- ggplot(meta, aes(Affection, chao1)) + 
  geom_boxplot(aes(color = Affection)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggsave("output/chao1.png", chao1_bp, height = 3, width = 3)


# For evenness
evenness_bp <- ggplot(meta, aes(Affection, pielou_evenness)) + 
  geom_boxplot(aes(color = Affection)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Evenness Index", x = "") 
ggsave("output/evenness.png", evenness_bp, height = 3, width = 3)


# For shannon
shannon_bp <- ggplot(meta, aes(Affection, shannon_entropy)) + 
  geom_boxplot(aes(color = Affection)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggsave("output/shannon.png", shannon_bp, height = 3, width = 3)

# For oberseved features

observed_features <- ggplot(meta, aes(Affection, observed_features)) + 
  geom_boxplot(aes(color = Affection)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Number of observed OTUs", x = "") 
