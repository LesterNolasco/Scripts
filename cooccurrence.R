...
ASVs <- read_qza("table-filtered1.qza")
...
dataset <- as.data.frame((ASV_table))
dataset[1:10,1:10]
dataset$ASVtotal <- rowSums(dataset)
dataset<-subset(dataset, ASVtotal>600)
dataset <- dataset[,!names(dataset) %in% "ASVtotal"]

dataset <- data.frame(r1=names(dataset), t(dataset))
dataset <- dataset[,-1]
...
treatments<-as.vector(unique(dataset$Affection))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$Affection)
...
head(final_results)
ggplot(gut_ASVs, aes(x = ASV7, y = ASV8)) +
  geom_point()
...
write.csv(subset(strong_results_taxa, trt=="low"), "output/moving-pictures-strong-results-taxa-low.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="high"), "output/moving-pictures-strong-results-taxa-high.csv", row.names = F)
