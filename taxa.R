...
#metadata<-read_q2metadata("matadata_assignment2.txt")
str(metadata)
#colnames(metadata)[3] = "body.site"
levels(metadata$Affection)
metadata$affection.ord = factor(metadata$Affection, c("low", "high"))
levels(metadata$affection.ord)
...
physeq <- qza_to_phyloseq(
  ...
  metadata = "metadata_assignment2.txt"
)
...
my_column <- "Affection"  #this is the metadata column that we will use in the taxa barplot
...
for(ml in my_level){
  ...
  physeq_meta_filtered$affection.ord = factor(physeq_meta_filtered$Affection, c("low", "high"))
  ...
  ggsave(paste0("output/", ml, "BarPlot_", my_column, ".png"), height = 5, width = 3.5)
}
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ Affection)
...
alpha = 0.05
my_contrast1 = c("Affection", "low", "high") 
...
res = results(diagdds, contrast = my_contrast1, cooksCutoff = FALSE)
...
ggsave(paste0("output/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 6, width = 30)

#my_colors[c(4,6,8,10,12,14,16,18,20)])